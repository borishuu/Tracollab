...
export default function Home() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            // Set le state uploading à true pour afficher le loader
            setUploading(true);
            ...
            // Set le state uploading à false pour cacher le loader
            setUploading(false);

            // Si la requête a réussi, rediriger l'utilisateur vers la page d'accueil
    ...
    // Fonction pour gérer l'upload du fichier audio
                ...
                <div className="lg:pl-12 lg:ml-24 sm:ml-0 sm:mr-0 lg:mr-24 lg:pr-12 lg:pt-3">
                    <div className="flex flex-col lg:flex-row h-full">
                        <div className="flex-none w-full lg:w-1/3 flex flex-col items-center pl-12 pr-12 pt-6">
                            <div
                                className="w-full max-w-xs aspect-square bg-gray-400 rounded-3xl flex justify-center items-center mb-4 relative">
                                {imagePreview ? (
                                    <img
                                        src={imagePreview}
                                        alt="Uploaded Image"
                        ...
                        <form onSubmit={handleSubmit} className="flex-1 h-full p-4 flex flex-col">
                            ...
                            <div className="h-1/6 mb-4 w-full">
                                ...
                                <DropDownList name={"All Genres"} data={genres}
                                              onChange={(e) => setSelectedGenre(e.target.value)} value={undefined}/>
                            </div>

                            <div className="h-1/6 mb-4 w-full">
                                <label className="block text-lg font-medium text-white">Type</label>
                                <DropDownList name={"Type"} data={[{name: "Instrumental"}, {name: "Voice"}]}
                                              onChange={(e) => setType(e.target.value)} value={undefined}/>
                            </div>
                            ...
                            <div
                                ...
                                <div
                                    className="flex flex-col lg:flex-row items-center space-y-2 lg:space-y-0 lg:space-x-4 w-full lg:w-auto">
                                    <label
